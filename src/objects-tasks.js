...
function removeProperties(obj, keys) {
  const res = Object.assign(JSON.parse(JSON.stringify(obj)));
  keys.forEach((key) => {
    delete res[key];
  });
  return res;
...
function compareObjects(obj1, obj2) {
  const res = JSON.stringify(obj1) === JSON.stringify(obj2); // true
  return res;
  // throw new Error('Not implemented');
}
...
class SelectorBuilder {
  ...
  element(value) {
    ...
    this.order = 1;
    this.selectors.set('element', true);
    this.result += value;
    return this;
  }
  ...
  pseudoElement(value) {
    ...
    this.order = 6;
    this.selectors.set('pseudoElement', true);
    this.result += `::${value}`;
    return this;
  }
